...
module.exports = async function runRateAndMessageMultipleLadies(page, tierConfigs) {
  ...
  const excludedLadyNames = new Set([
    ...
    'Michel','אלכס','Uruguaya','Jaisita','X_Haiko_X','Saddalyn','sa.ahil','MintyRose','kaya21','baraa bf','mnijeonhaneen','TheResa75','Méllisa','Lady Abbie Queen','Abc','ساجده','Sharisse','...','Pamuk','Maya','R H A E N Y R A',
    // add all names you want to exclude
  ].map(n => n.toLowerCase())); // convert to lowercase for case-insensitive matching

  const m1 = 'Sending positive vibes! 🌿🍒 Have a cherry-fic day my friend! (｡♥‿♥｡)';
  const m2 = 'Sending positive vibes! 🌿🍒 Have a cherry-fic day my friend! (｡♥‿♥｡)'; //already won
  const m3 = 'Sending positive vibes! 🌿🍒 Have a cherry-fic day my friend! (｡♥‿♥｡)'; //168
