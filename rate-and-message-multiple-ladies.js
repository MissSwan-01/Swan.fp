...
module.exports = async function runRateAndMessageMultipleLadies(page, tierConfigs) {
  ...
  const excludedLadyNames = new Set([
    ...
    'laura','Ella','Jurria','Gloria','welle','Radha','Rosalina','Funny Face','reyhan','Yuna','Isabel','steph','maryann','Lovlee','Lana','-𝐓𝐫𝐢𝐤𝐢𝐳𝐳𝐙-','Yeluna Corazón','x_.sono.prvt_','sabrina13','Jade','VenusDoom','Mikel','Singin-Suzun',
    // add all names you want to exclude
  ].map(n => n.toLowerCase())); // convert to lowercase for case-insensitive matching

  const m1 = 'Happy weekend, beautiful! 🐻 You are bear-y pretty! Max stars ^^';
  const m2 = 'Happy weekend, beautiful! 🐻 You are bear-y pretty!'; //already won
  const m3 = 'Happy weekend, beautiful! 🐻 You are bear-y pretty!'; //168
