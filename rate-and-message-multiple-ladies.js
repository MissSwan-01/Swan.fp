...
module.exports = async function runRateAndMessageMultipleLadies(page, tierConfigs) {
  ...
  const excludedLadyNames = new Set([
    ...
    'Michel','אלכס','Uruguaya','Jaisita','X_Haiko_X','Saddalyn','sa.ahil',
    // add all names you want to exclude
  ].map(n => n.toLowerCase())); // convert to lowercase for case-insensitive matching

  const m1 = 'Sending positive vibes! Have a pleasant weekend my friend. 🐾 You are pure paw-fection! ฅ^>⩊<^ ฅ';
  const m2 = 'Sending positive vibes! Have a pleasant weekend my friend. 🐾 You are pure paw-fection! ฅ^>⩊<^ ฅ'; //already won
  const m3 = 'Sending positive vibes! Have a pleasant weekend my friend. 🐾 You are pure paw-fection! ฅ^>⩊<^ ฅ'; //168
